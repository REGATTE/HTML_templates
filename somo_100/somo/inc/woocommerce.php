 ...
 **/
if (!function_exists('somo_woocommerce_template_loop_product_thumbnail')) {
  function somo_woocommerce_template_loop_product_thumbnail() {
    echo somo_woocommerce_get_product_thumbnail();
  }
}

if (!function_exists('somo_woocommerce_get_product_thumbnail')) {

  function somo_woocommerce_get_product_thumbnail($size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0) {
    ...
    $label = yprm_get_theme_setting('tr_purchase');
    ...
    if ($product->is_on_sale()) {
      $output .= '<span class="onsale accent"><span>' . esc_html__('Sale', 'somo') . '</span></span>';
    }
    ...
    if (class_exists('YPRM_Somo_Addons') && $attachment_ids = $product->get_gallery_image_ids()) {
      ...
      if (has_post_thumbnail()) {
        ...
        $output .= '<img src="' . wc_placeholder_img_src() . '" alt="' . esc_attr__('Placeholder', 'somo') . '" width="' . $placeholder_width . '" height="' . $placeholder_height . '" />';
      }
      $output .= '</a>';
    }
    $output .= '</div>';
...
add_action('woocommerce_before_shop_loop_item_title', 'somo_woocommerce_template_loop_product_thumbnail', 10);
...
function woocommerce_header_add_to_cart_fragment($fragments) {
  ...
  $fragments['.header-minicart-somo'] = ob_get_clean();
  return $fragments;
}
...
add_filter('woocommerce_output_related_products_args', 'somo_single_products_args', 20);
add_filter('woocommerce_upsell_display_args', 'somo_single_products_args', 20);
function somo_single_products_args($args) {
...
add_filter('woocommerce_cross_sells_columns', 'somo_woocommerce_cross_sells_columns', 20);
function somo_woocommerce_cross_sells_columns() {
  return 1;
}
...
function yprm_button_view_cart() {
  ...
  echo '<a href="' . esc_url($link) . '" class="button-style1"><span>' . esc_html__('View cart', 'somo') . '</span></a>';
}

function yprm_proceed_to_checkout() {
  $link = wc_get_checkout_url();
  echo '<a href="' . esc_url($link) . '" class="button-style1 checkout accent"><span>' . esc_html__('Checkout', 'somo') . '</span></a>';
}
